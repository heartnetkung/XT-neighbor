...
/**threshold for static memory allocation */
const int MAX_DISTANCE1 = 2;
...
#endif
__device__
void expand_keys_static1(Int3 seq, int distance, Int3* output, unsigned int* firstKeys, int start, int end) {
	int len = len_decode(seq);
	const int effectiveDistance = distance < len ? distance : len;

	//init stacks
	int indexStack[MAX_DISTANCE1 + 1];
	Int3 seqStack[MAX_DISTANCE1 + 1];
	seqStack[0] = seq;
	for (int i = 1; i <= effectiveDistance; i++) {
		indexStack[i] = i - 1;
		seqStack[i] = remove_char(seqStack[i - 1], 0);
...
#endif
__device__
void expand_keys_static2(Int3 seq, int distance, Int3* output, unsigned int* firstKeys, int start, int end) {
	int len = len_decode(seq);
	const int effectiveDistance = distance < len ? distance : len;

	//init stacks
	int indexStack[MAX_DISTANCE2 + 1];
	Int3 seqStack[MAX_DISTANCE2 + 1];
	seqStack[0] = seq;
	for (int i = 1; i <= effectiveDistance; i++) {
		indexStack[i] = i - 1;
		seqStack[i] = remove_char(seqStack[i - 1], 0);
...
void gen_combination(Int3* seqs, int* combinationOffsets, int distance,
	...
	if (distance <= MAX_DISTANCE1)
		expand_keys_static1(seqs[tid], distance, combinationKeys, firstKeys, start, end);
	else if (distance <= MAX_DISTANCE2)
		expand_keys_static2(seqs[tid], distance, combinationKeys, firstKeys, start, end);
	else
